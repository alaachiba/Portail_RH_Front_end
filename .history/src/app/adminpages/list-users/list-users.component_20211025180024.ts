...
export class ListUsersComponent implements OnInit {
  ...
  onAddConfirm(event): void {
    ...
    if (window.confirm("Êtes-vous sûr d'ajouter l'utilisateur ?")) {
      if (event.newData.nom && 
        ...
        event.newData.roles !=-1
          ...
          this.service.CreateUser(
            ...
            event.newData.matricule,
            event.newData.email,
            this.listRoles[event.newData.roles].libelle,
            "0",
  ...
  onUpdateConfirm(event): void {
    ...
    if (window.confirm("Êtes-vous sûr de modifier l'utilisateur ?")) {
  ...
  onDeleteConfirm(event): void {
    ...
    if (window.confirm("Êtes-vous sûr de retirer l'utilisateur ?")) {
      console.log(event.data.idUser);
